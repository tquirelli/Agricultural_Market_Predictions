...
from .preprocesador_corn import all_preprocessor_xgb_corn, all_preprocessor_train_xgb_corn
from .xgboost_corn import initialize_train_model, mape_score, MAPE_validation
...
#csv_path = os.path.join(script_path, "..", LOCAL_PATH_corn)
...
def input_usuario_corn(N_month_predict:int, consolidado_corn):
    # verificar que se encuentre en la base de datos
    df = all_preprocessor_xgb_corn(consolidado_corn) # preprocesando el dataframe completo
...
def feature_selection(df, N_month_predict):
    ...
    Variables['y_{}'.format(N_month_predict)] = np.log(df['price_corn'])
    Selected_features = [df.columns.tolist()[i] for i in range(16, df.shape[1])]
    Selected_features.append('{} (t-{})'.format('price_corn', N_month_predict))
    ...
    Selected_features.append('{} (t-{})'.format('usa_corn_prod', N_month_predict))
    Variables['X_{}'.format(N_month_predict)] = df[Selected_features]
    return Variables

def train_xgboost(N_month_predict, consolidado_corn) -> pd.DataFrame:
    df = all_preprocessor_train_xgb_corn(consolidado_corn)
...
def predict_xgboost_corn(filtered_df, N_month_predict):
    ...
    try:
        ...
        print("El precio del maíz dentro de {} meses será de $: ".format(N_month_predict),price_soybean_result)
        return str(price_soybean_result)

    except FileNotFoundError:
        train_xgboost(N_month_predict, consolidado_corn)
...
if __name__ == '__main__':
    try:
        ...
        predict_xgboost_corn(filtered_df, N_month_predict)
